                ...
                <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="password" name="password"
                            class="form-input @error('password') border-rose-500 @enderror" required placeholder="••••••••">
                    ...
                    <div class="flex items-center pt-2">
                        <input type="checkbox" id="is_active" name="is_active" value="1"
                            class="w-4 h-4 rounded border-slate-300 text-emerald-600" checked>
                    ...
                    <div>
                        ...
                        <select name="allowed_tenor" id="allowed_tenor" class="form-select @error('allowed_tenor') border-rose-500 @enderror" required>
                            <option value="3" {{ old('allowed_tenor', 3) == 3 ? 'selected' : '' }}>3 Bulan (Default)</option>
                            <option value="6" {{ old('allowed_tenor') == 6 ? 'selected' : '' }}>6 Bulan</option>
                            <option value="12" {{ old('allowed_tenor') == 12 ? 'selected' : '' }}>12 Bulan</option>
                        </select>
                        @error('allowed_tenor')
                            <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="pt-2 flex justify-end gap-2 border-t border-slate-200">
                        ...
                        <button type="submit" class="btn-primary text-xs">Simpan Akun</button>
                    </div>
                </form>
            </div>
        </div>
